          ...
          child: BlocBuilder<ElementsBloc, ElementsState>(
            builder: (context, state) {
              ...
              if (state is ElementsLoaded) {
                ...
                // Mapeo rápido de elementos por posición
                final elementMap = {
                  for (final el in state.all) '${el.x},${el.y}': el
                };
                final filteredIds = state.filtered.map((e) => e.id).toSet();
                final isSearching = state.query.isNotEmpty;

                final maxRow = state.all
                    .map((e) => e.y)
                    .fold(4, (max, y) => y > max ? y : max);
                      ...
                      child: SizedBox(
                        ...
                        child: GridView.builder(
                          ...
                          itemBuilder: (context, i) {
                            ...
                            final matches =
                                !isSearching || filteredIds.contains(el.id);
